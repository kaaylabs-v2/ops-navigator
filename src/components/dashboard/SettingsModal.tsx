...
export function SettingsModal({ open, onClose, onSaved }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 animate-fade-up"
      style={{ background: "oklch(0.08 0.02 280 / 0.7)", backdropFilter: "blur(8px)" }}
      onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()}
        className="card-surface w-full max-w-4xl max-h-[88vh] flex flex-col overflow-hidden"
        style={{ background: "oklch(0.18 0.025 280 / 0.95)" }}>
        ...
        <div className="flex flex-1 min-h-0">
          <nav className="w-[200px] border-r border-white/8 p-3 space-y-1 shrink-0">
                  ...
                  <div className="space-y-3 animate-fade-up">
                    ...
                    <div className="flex items-center gap-3 pt-1">
                      <button onClick={runTest} disabled={testing}
                        className="px-3 py-2 text-xs font-medium border border-white/15 rounded-lg hover:border-white/30 transition-colors disabled:opacity-50">
                        {testing ? "Testing…" : "Test connection"}
                      </button>
                      {testResult && (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs"
                          style={{
                            background: testResult.ok ? "oklch(0.72 0.18 155 / 0.15)" : "oklch(0.66 0.22 18 / 0.15)",
              ...
              <div className="space-y-4 text-sm leading-relaxed">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {["Frontend", "Backend", "AI: Claude path", "AI: Nexus path"].map((c) => (
                    <span key={c} className="px-2 py-0.5 rounded-md text-xs border border-white/10 bg-white/5">{c}</span>
                  ))}
                </div>
        ...
        <div className="flex justify-end gap-2 px-6 py-4 border-t border-white/8">
          <button onClick={onClose} className="px-4 py-2 text-xs font-medium text-muted-foreground hover:text-foreground rounded-lg hover:bg-white/5 transition-colors">
            Cancel
          </button>
  ...
  return (
    <button onClick={onClick}
      className={`text-left p-4 rounded-xl border transition-all ${
        active ? "bg-white/5" : "border-white/8 hover:border-white/20"
      }`}
      style={active ? { borderColor: "oklch(0.66 0.22 290 / 0.5)", boxShadow: "0 0 0 1px oklch(0.66 0.22 290 / 0.3)" } : undefined}>
  ...
  return (
    <div className={col}>
      ...
      <input type={type} value={value} placeholder={placeholder}
        ...
        className={`w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-white/30 ${mono ? "mono" : ""}`} />
    </div>
  );
}
